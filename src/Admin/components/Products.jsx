...
const rows = [
  { id: 1, name: "Cotton Polo T-Shirt", sku: "TSH001", category: "T-Shirts", brand: "Allen Solly", size: "M, L, XL", price: "799.00", stock: 45, status: "Active", color: ["#1976d2", "#000", "#9e9e9e"] },
  { id: 2, name: "Denim Jeans", sku: "JN001", category: "Jeans", brand: "Levis", size: "30, 32, 34, 36", price: "1,299.00", stock: 30, status: "Active", color: ["#1976d2", "#9e9e9e", "#000"] },
      ...
      <Box sx={{ width: '100%', mt: 4, p: 2 }}>
        <Grid container spacing={2} alignItems="center">
          ...
          {statsData.map((item, index) => (
            <Grid item key={index} xs={12} sm={6} md={2.6}>
              <Paper
                elevation={0}
                sx={{
                  p: 2,
            ...
            <TableBody>
              {rows.map((row) => (
                <TableRow key={row.id} hover>
                  <TableCell padding="checkbox"><Checkbox size="small" /></TableCell>
                  <TableCell><Avatar variant="rounded" sx={{ width: 40, height: 40, bgcolor: "#F3F4F6" }} /></TableCell>
                  <TableCell>
                    <Typography sx={{ fontWeight: 600, fontSize: "0.85rem" }}>{row.name}</Typography>
                    <Typography variant="caption" color="text.secondary">090123456789</Typography>
                  </TableCell>
                  <TableCell sx={{ fontSize: "0.85rem" }}>{row.sku}</TableCell>
                  <TableCell>
                    <Chip label={row.category} size="small" sx={{ bgcolor: "#F5F3FF", color: "#7C3AED", fontWeight: 600 }} />
                  </TableCell>
                  ...
                  <TableCell>
                    <Stack direction="row" spacing={0.5}>
                      {row.color.map((c, i) => (
                        <Box key={i} sx={{ width: 12, height: 12, borderRadius: "50%", bgcolor: c, border: "1px solid #E5E7EB" }} />
                      ))}
                    </Stack>
                  </TableCell>
                  <TableCell sx={{ fontWeight: 700 }}>{row.price}</TableCell>
                  <TableCell sx={{ fontWeight: 700, color: "#16A34A" }}>{row.stock}</TableCell>
                  <TableCell>
                    <Chip label={row.status} size="small" color="success" variant="soft" sx={{ borderRadius: "6px" }} />
                  </TableCell>
                  <TableCell>
                    <Stack direction="row">
                      <IconButton size="small" color="primary"><EditOutlinedIcon fontSize="small" /></IconButton>
                      <IconButton size="small" color="error"><DeleteOutlineOutlinedIcon fontSize="small" /></IconButton>
                    </Stack>
                  </TableCell>
                </TableRow>
              ))}
